...
@threestudio.register("neus-volume-renderer2")
class NeuSVolumeRenderer(VolumeRenderer):
    ...
    def forward(
        ...
        hits: Float[Tensor, "B H W 3"] = None,
        x: Float[Tensor, "B H W 3"] = None,
        **kwargs
        ...
        if hits is None:
            ...
            # todo: create t_starts, t_ends and ray_indices according to hits tensor
            pass
            batch_size,height,width = hits.shape[:3]
            ray_indices = torch.arange(width*height*batch_size, device = hits.device).reshape([batch_size,width,height])[hits]
            ray_indices = ray_indices.repeat_interleave(2)
            origins = rays_o[:,:1,:1,:].repeat([1,width, height,1])
            n_rays = rays_o_flatten.shape[0]
            vecs = (x - origins)[hits]
            t = torch.norm(vecs, dim=-1)
            prev_t = t-0.1 # TODO: make hyperparameter
            next_t = t+0.1
            t_starts_ = t.repeat_interleave(2)
            t_starts_[::2] = prev_t
            t_ends_ = t.repeat_interleave(2)
            t_ends_[1::2] = next_t
            light_positions_flatten: Float[Tensor, "Nr 3"] = (
            light_positions.reshape(-1, 1, 1, 3)
            .expand(-1, height, width, -1)
        ...
        depth: Float[Tensor, "Nr 1"] = nerfacc.accumulate_along_rays(
            weights[..., 0], values=t_positions**-1, ray_indices=ray_indices, n_rays=n_rays
        )
